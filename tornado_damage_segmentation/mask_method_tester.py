...
def create_masks(rasters, new=False):
    ...
    for raster in rasters:
        ...
        with rasterio.open('ForWarn_2021' + added + '_masks' + '/' + raster.name.split('/')[-1], 'w', **out_meta) as \
                dest:
            dest.write(out_img)
            dest.close()
...
# Loads rasters
def load_rasters(dir):
    return [rasterio.open(filename) for filename in os.listdir(dir)]


old_rasters = load_rasters('C:/Users/CyborgOctopus/ForWarn.2021_X_LC_5YEAR.tar')
